...
class TestOrderPage:
    ...
    @allure.description ('Заполняем форму заказа самоката из двух частей и убеждаемся в позитивном итоге заполнения')
    def test_order_page_version_1(self,driver):
    ...
    @allure.description ('Заполняем форму заказа самоката из двух частей и убеждаемся в позитивном итоге заполнения')
    def test_order_page_version_2(self, driver):
        order_page = OrderPage(driver)
        order_page.click_cookie()
        order_page.click_order_button()
